...
/**
 * CRUD service for remote RPM repositories (upstream yum/dnf mirrors).
 * Base path: {@code /repositories/rpm/remote}
 */
@Path( "repositories/rpm/remote" )
@Tag( name = "v2" )
@Tag( name = "v2/Repositories" )
public interface RpmRemoteRepositoryService
{
    @GET
    @Produces( {APPLICATION_JSON} )
    @RedbackAuthorization( permissions = {OPERATION_MANAGE_CONFIGURATION} )
    @Operation( summary = "Returns all remote RPM repositories.",
        parameters = {
        ...
        responses = {
            @ApiResponse( responseCode = "200", description = "List of remote RPM repositories" ),
            @ApiResponse( responseCode = "403", description = "Authenticated user does not have permission" )
        }
    )
    PagedResult<RpmRemoteRepository> getRemoteRepositories(
    ...
    @Produces( {APPLICATION_JSON} )
    @RedbackAuthorization( permissions = {OPERATION_MANAGE_CONFIGURATION} )
    @Operation( summary = "Returns a remote RPM repository by id.",
        security = {@SecurityRequirement( name = OPERATION_MANAGE_CONFIGURATION )},
        responses = {
            @ApiResponse( responseCode = "200", description = "The remote RPM repository" ),
            @ApiResponse( responseCode = "403", description = "Authenticated user does not have permission" ),
            @ApiResponse( responseCode = "404", description = "Repository not found" )
        }
    )
    RpmRemoteRepository getRemoteRepository( @PathParam( "id" ) String repositoryId )
        throws ArchivaRestServiceException;
    ...
    @Produces( {APPLICATION_JSON} )
    @RedbackAuthorization( permissions = {OPERATION_MANAGE_CONFIGURATION} )
    @Operation( summary = "Creates a new remote RPM repository.",
        security = {@SecurityRequirement( name = OPERATION_MANAGE_CONFIGURATION )},
        responses = {
    ...
    RpmRemoteRepository addRemoteRepository( RpmRemoteRepository remoteRepository )
        throws ArchivaRestServiceException;
    ...
    @Produces( {APPLICATION_JSON} )
    @RedbackAuthorization( permissions = {OPERATION_MANAGE_CONFIGURATION} )
    @Operation( summary = "Updates an existing remote RPM repository.",
        security = {@SecurityRequirement( name = OPERATION_MANAGE_CONFIGURATION )},
        responses = {
    ...
    RpmRemoteRepository updateRemoteRepository( @PathParam( "id" ) String repositoryId,
                                               RpmRemoteRepository remoteRepository )
        throws ArchivaRestServiceException;
    ...
    @Produces( {APPLICATION_JSON} )
    @RedbackAuthorization( permissions = {OPERATION_MANAGE_CONFIGURATION} )
    @Operation( summary = "Deletes a remote RPM repository.",
        security = {@SecurityRequirement( name = OPERATION_MANAGE_CONFIGURATION )},
        responses = {
