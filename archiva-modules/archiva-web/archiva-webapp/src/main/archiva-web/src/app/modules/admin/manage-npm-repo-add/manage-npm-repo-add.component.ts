...
@Component({
    selector: 'app-manage-npm-repo-add',
    templateUrl: './manage-npm-repo-add.component.html'
})
export class ManageNpmRepoAddComponent implements OnInit {
    ...
    onSubmit(): void {
        ...
        const repo = Object.assign(new NpmManagedRepository(), this.repoForm.value);
        this.npmService.addRepository(repo).pipe(
            catchError((err: ErrorResult) => {
                this.errorResult = err;
                this.success = false;
                this.error = true;
                this.toastService.showError('npm-repo-add', this.errorTmpl, {contextData: err});
                return [];
            })
        ).subscribe((created: NpmManagedRepository) => {
            this.result = created;
            this.success = true;
            this.error = false;
            this.toastService.showSuccess('npm-repo-add', this.successTmpl, {contextData: created});
