...
@Component({
    selector: 'app-manage-rpm-remote-repo-list',
    templateUrl: './manage-rpm-remote-repo-list.component.html'
})
export class ManageRpmRemoteRepoListComponent implements OnInit {

    @ViewChild('deletedTmpl') public deletedTmpl: TemplateRef<any>;
    @ViewChild('errorTmpl') public errorTmpl: TemplateRef<any>;

    service: EntityService<RpmRemoteRepository>;
    sortField = ['id'];
    sortOrder = 'asc';
    ...
    constructor(private rpmService: RpmRepositoryService, private toastService: ToastService) {
        this.service = (searchTerm: string, offset: number, limit: number,
            ...
            rpmService.queryRemote(searchTerm, offset, limit, orderBy, order);
    }

    ngOnInit(): void {}

    deleteRepository(repo: RpmRemoteRepository): void {
        this.rpmService.deleteRemoteRepository(repo.id).pipe(
            catchError((error: ErrorResult) => {
                this.toastService.showError('rpm-remote-repo-list', this.errorTmpl, {contextData: error});
                return [];
            })
        ).subscribe(() => {
            this.toastService.showSuccess('rpm-remote-repo-list', this.deletedTmpl, {contextData: repo});
        });
    }
}
